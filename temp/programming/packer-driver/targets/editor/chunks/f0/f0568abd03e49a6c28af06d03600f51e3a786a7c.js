    ...
    execute: function () {
      ...
      _export("EnemyController", EnemyController = (_dec = ccclass('EnemyController'), _dec2 = property({
        ...
        update(deltaTime) {
          ...
          let currentPos = this.node.worldPosition; // Вычисляем вектор направления к цели

          let direction = new Vec3();
          Vec3.subtract(direction, this.targetPosition, currentPos); // Узнаем расстояние до цели

          let distance = direction.length(); // Если враг дошел до точки (расстояние меньше 5 пикселей), удаляем его

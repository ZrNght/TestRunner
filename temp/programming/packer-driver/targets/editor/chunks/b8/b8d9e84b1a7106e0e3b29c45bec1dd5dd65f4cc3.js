    ...
    execute: function () {
      ...
      _export("CursorPulse", CursorPulse = (_dec = ccclass('CursorPulse'), _dec2 = property({
        ...
        start() {
          ...
          const originalScale = this.node.scale.clone(); // Вычисляем целевой (увеличенный) масштаб

          const targetScale = new Vec3(originalScale.x * this.scaleFactor, originalScale.y * this.scaleFactor, originalScale.z); // Запускаем бесконечную анимацию пульсации

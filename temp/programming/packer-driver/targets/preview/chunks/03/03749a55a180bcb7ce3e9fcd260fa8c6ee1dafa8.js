    ...
    execute: function () {
      ...
      _export("CashSpawner", CashSpawner = (_dec = ccclass('CashSpawner'), _dec2 = property({
        ...
        spawnSingle() {
          ...
          var prefab = this.prefabsToSpawn[Math.floor(Math.random() * this.prefabsToSpawn.length)];
          var spawnedObj = instantiate(prefab);
        ...
        spawnArch() {
          ...
          for (var i = 0; i < this.archHeights.length; i++) {
            var prefab = this.prefabsToSpawn[Math.floor(Math.random() * this.prefabsToSpawn.length)];
            var spawnedObj = instantiate(prefab);
            this.node.addChild(spawnedObj);
